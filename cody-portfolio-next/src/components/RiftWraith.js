export function createRiftWraith(ctx, w, h, DPR, particles, enabled, isMobile, mouse) {
    const SPRITES = {
        creature: [
        ...
        if (creature.active) {
                ...
                if (now > creature.nextSeekAt) {
                    if (!creature.target || now > creature.nextTargetSwitch) {
                        ...
                        for (let i = 0; i < particles.length; i++) {
                            ...
                            const dx = px - creature.x;
                            const dy = py - creature.y;
                    ...
                    if (creature.target) {
                        const tp = creature.target;
                        if (!tp) return;
                        const parallaxX = !isMobile && enabled ? (mouse?.x - 0.5) * tp.z * 0.12 : 0;
                        const parallaxY = !isMobile && enabled ? (mouse?.y - 0.5) * tp.z * 0.12 : 0;
                        const tx = tp.x + parallaxX;
                        const ty = tp.y + parallaxY;
            ...
            if (levelUpBursts.length) {
                ...
                for (let i = levelUpBursts.length - 1; i >= 0; i--) {
                    ...
                    b.r += 8 * (0.5 + b.scale);
                    b.life -= 0.04;
                    if (b.life <= 0) { levelUpBursts.splice(i, 1); continue; }
            ...
            creature.angle += diff * 0.08;

            const wiggle = Math.sin(Date.now() * 0.006) * 0.15;
            ctx.translate(dx, dy);
            ctx.rotate(creature.angle + wiggle);
            if (img && img.complete) ctx.drawImage(img, -s, -s, s * 2, s * 2);
            ctx.restore();
        }
    }
